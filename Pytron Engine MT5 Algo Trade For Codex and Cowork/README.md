...
-->
# Pytron Engine MT5 Algo Trade — for Codex · Cowork · Cursor
...
├── SKILL.md                     ← จุดเข้าใช้งานสำหรับ AI (Codex · Cowork · Cursor)
...
├── scripts/verify_package.py    ← ตรวจความสมบูรณ์ + ตรวจว่าไม่มีความลับ
...
**กลุ่มค่า (อ่านรายละเอียดในไฟล์จริง):**

- `m1_trigger` — เงื่อนไขเข้าออกบนแท่ง M1 (RSI ฝั่งซื้อ/ขาย + ตำแหน่งปิดแท่ง)
- `legacy_trend` — ระบบโหวตทิศทางรุ่นเดิม (จำนวนโหวต + การยืนยัน H1 + กรอบ RSI)
- `strategy_router` — เราเตอร์เลือกกลยุทธ์ (โหมด `technical_only`, เปิด/ปิดรายกลยุทธ์)
- `profit_exit` — ปิดทำกำไรเมื่อสัญญาณทางเดียวกันหมด (สัดส่วนของระยะ TP)
- `openrouter` — บอท AI ผ่าน OpenRouter (**`enabled: false`** เป็นค่าเริ่มต้น)
- `bounded_adaptive_research` — วิจัยปรับตัวแบบมีขอบเขต (`shadow_only`, ไม่แตะการเทรดจริง)
- `side_net_gate` — ด่านเน็ตรายฝั่ง (ดูไม้ที่ปิดใน 12 ชม. ล่าสุด)
- `stage3` — ด่านที่ 3: เทียบสองฝั่งด้วย probability + weighted score
- `revenge_guard` — กันเข้าไม้ซ้ำฝั่งที่เพิ่งขาดทุน (คูลดาวน์)
- `early_cut` — ตัดขาดทุนเมื่อไม้เดิมขาดทุนและสัญญาณใหม่สวนทาง
- `admin_bot` — บอท AI ดูแลระบบเป็นรอบ (`interval_minutes`, โหมด `apply`)
...
```bash
python scripts/verify_package.py            # 1) ตรวจความสมบูรณ์ + ไม่มีความลับ
#                                          2) ติดตั้งสำเนาทำงาน (ดู SKILL.md หัวข้อ 2)
python outputs/mt5_python_bridge/mt5_probe.py         # 3) ตรวจเทอร์มินัล/โบรก
...
| ไม่มี terminal (คลาวด์/ลินุกซ์/Cowork) | `metaapi/` (สะพาน MetaAPI) | ✅ ทดสอบเชื่อมต่อจริง อ่านอย่างเดียว 21 ก.ย. 2026 |
| Codex · Cursor · Cowork | อ่าน `SKILL.md` + `agents/registry.json` | ✅ |

ข้อจำกัดจริง: hosted Cowork ไม่มีเทอร์มินัล MT5 ของ Windows จึงใช้เส้นทาง MetaAPI
และ MetaAPI **ต้องมีบัญชี + โทเคนของคุณเอง** — แพ็กนี้ไม่แถมและไม่ฝัง credential ของผู้ใด
...
| รอบ | วิธี | ผล |
|---|---|---|
| รอบ 1 | สคริปต์ตรวจของเจ้าของระบบ (16 หัวข้อ: โครงสร้าง · ความลับ · เทสต์ · ค่าโรงงาน · manifest) | 16/16 ผ่าน |
...
- ห่วงโซ่แฮชหลักฐานใน `research/recommendations/evaluations/` ตรวจซ้ำแล้ว **ok=300 broken=0**
  (ทุก `verdict.json` ตรงกับ `recommendation.json` และ `baseline.json` ของตัวเอง)
- คู่ `work/factory/code/` กับโค้ดจริง **ไม่ตรง 12 คู่** — เป็นสภาพจริงของระบบตั้งแต่ต้น
  (ตัวคืนค่าโรงงานเก็บสำเนาที่ต่างจากตัวจริงอยู่แล้ว) ไม่ใช่ความเสียหายจากแพ็กนี้
