...
def decide_market(frames: dict, config: dict) -> dict:
    ...
    if not directional_scores:
        return {
            ...
            "reason": ("validation-locked: all strategies are disabled"
                        if any(candidate.get("eligible") for candidate in candidates)
                        else "no positive Agent score available"), "stop_distance": None,
