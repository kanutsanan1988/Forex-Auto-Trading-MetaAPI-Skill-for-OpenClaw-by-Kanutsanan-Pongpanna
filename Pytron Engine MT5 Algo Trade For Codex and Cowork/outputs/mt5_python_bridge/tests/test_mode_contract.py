...
class ModeContractTests(unittest.TestCase):
    ...
    def test_internal_keeps_python_and_pauses_every_registered_ai_job(self):
        with patch.object(modes, 'cron') as cron:
            modes.resume_selected('internal_only')
        actions = [c.args for c in cron.call_args_list]
        self.assertEqual({n for a, n in actions if a == 'resume'}, set(modes.PYTHON_JOBS))
        self.assertEqual({n for a, n in actions if a == 'pause'}, set(modes.AI_JOBS))
        self.assertIn('brain-consult', modes.AI_JOBS)

    def test_ai_keeps_python_and_enables_every_registered_ai_job(self):
        with patch.object(modes, 'cron') as cron:
    ...
    def test_project_python_console_uses_utf8(self):
        stream = unittest.mock.Mock()
        support.configure_utf8_stdio((stream,))
        stream.reconfigure.assert_called_once_with(encoding='utf-8', errors='replace')

    def test_jev_trading_call_obeys_global_ai_mode_guard(self):
        jev = load_module('test_jev_mode_gate', BR / 'tools' / 'jev.py')
        with patch.object(jev, 'require_ai_mode', side_effect=ValueError('mode 1')), \
             patch.object(jev, '_post') as post:
            result = jev.ask({'test': True}, [], context='mode2')
        self.assertFalse(result['ok'])
        self.assertIn('mode 1', result['error'])
    ...
    def test_jev_missing_connection_is_optional_and_returns_unavailable(self):
        ...
        self.assertFalse(result['ok'])
        self.assertIn('ไม่พบคีย์', result['error'])
        post.assert_not_called()
