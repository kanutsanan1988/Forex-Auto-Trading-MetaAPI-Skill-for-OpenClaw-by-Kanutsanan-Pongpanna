...
LOG_FILE = "/var/log/trade_check.log"
...
def manual_trade_check():
    log("=" * 60)
    log("MANUAL TRADE CHECK (OpenRouter AI Analysis Only)")
    log("=" * 60)
    ...
    # Step 8: Manual Check Mode - NO EXECUTION
    log(f"Step 8: [MANUAL MODE] Would execute {signal} {lot} lot @{entry:.3f} SL:{sl} TP:{tp}")
    return f"ANALYSIS COMPLETE: {signal} {lot}lot @{entry:.3f} SL:{sl} TP:{tp} Str:{strength}/10 Src:{data_source}"
